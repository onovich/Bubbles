...
namespace Bubbles {
    ...
    public static class GameMapDomain {
        ...
        static void DrawAllPath(GameBusinessContext ctx) {
            var enemyLen = ctx.bubbleRepo.TakeAll(out var enemyArr);
            for (int i = 0; i < enemyLen; i++) {
                var bubble = enemyArr[i];
                if (bubble.allyStatus != AllyStatus.Enemy) {
                    continue;
                }
                var path = bubble.path;
                var pathLen = bubble.pathLen;
                if (pathLen > 0) {
